...
export const LiveTraceView: React.FC = () => {
  ...
  useEffect(() => {
    clearStore(); // reset stale task state from previous run
    if (id) {
      connect(id, token);
    }
    return () => {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-[32px] font-light text-ink tracking-[-0.64px]">Swarm Run: <span className="tabular-nums-money">{id?.slice(0, 8)}...</span></h2>
        ...
        <div className="flex gap-2">
          ...
          <Link to="/dashboard" className="rounded-[6px] border border-dark-border p-[8px_16px] text-[15px] font-light text-ink hover:bg-dark-border/40 transition-all flex justify-center items-center">
            Back
          </Link>
        </div>
      </div>
      ...
      {swarmStatus === 'completed' && finalOutput && (
        <div className="mt-8 bg-dark-card border border-primary/20 rounded-[12px] p-6 shadow-[rgba(0,55,112,0.08)_0_8px_24px,rgba(0,55,112,0.04)_0_2px_6px]">
          <h3 className="text-lg font-medium text-ink mb-4">Final Result</h3>
          <div className="bg-dark-bg p-4 rounded-md border border-dark-border text-ink-mute text-sm whitespace-pre-wrap font-mono overflow-auto max-h-[400px]">
            {finalOutput}
          </div>
        </div>
      )}
