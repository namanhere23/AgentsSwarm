...
async def test_whisper_worker_transcribes_mock_wav():
    ...
    with patch(
        "backend.workers.whisper_consumer.AsyncGroq", return_value=mock_groq
    ), patch("backend.workers.whisper_consumer.subprocess.run") as mock_sub, patch(
        "backend.workers.whisper_consumer.os.path.exists", return_value=True
    ), patch(
        "backend.workers.whisper_consumer.os.remove"
    ), patch(
