...
async def upload_voice_objective(
    ...
    if content_type not in allowed_types:
        raise HTTPException(status_code=400, detail="MIME type must be WAV, OGG, or WEBM audio formats.")

    # Validate size (100MB ceiling)
    audio_data = await audio.read()
    if len(audio_data) > 100 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Audio file exceeds 100MB size limit.")
    ...
    await event_bus.publish("stt_queue", {
