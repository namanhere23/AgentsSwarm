...
class SwarmWatchdog:
    ...
    def check(self) -> bool:
        ...
        if ratio >= 0.90 and not self._warned_90:
            ...
            logger.warning(f"Watchdog: 90% time used ({elapsed:.0f}s). {(self.deadline - now):.1f}s remaining.")
        elif ratio >= 0.75 and not self._warned_75:
            self._warned_75 = True
            logger.info(f"Watchdog: 75% time used. {(self.deadline - now):.1f}s remaining.")

        if now > self.deadline:
            raise SwarmTimeoutError(f"Swarm exceeded {self.max_wall_time_seconds}s wall time. Total elapsed: {elapsed:.1f}s.")
        return True

    def get_remaining_budget_tokens(self, base_budget: int) -> int:
        ratio = (time.time() - self.start_time) / self.max_wall_time_seconds
        if ratio >= 0.90: scale = 0.30
        elif ratio >= 0.75: scale = 0.60
        elif ratio >= 0.50: scale = 0.80
        else: scale = 1.0
